"""Typed provenance and audit evidence outside Canonical semantics.
...
class NativeLocatorKind(StrEnum):
...
class NativeLocator(CanonicalModel):
    """A typed native identity, never a host filesystem path."""

    kind: NativeLocatorKind
    ...
    def validate_locator_value(cls, value: str, info: ValidationInfo) -> str:
        ...
        if kind is NativeLocatorKind.SCENE_GRAPH_PATH:
...
class MappingProofKind(StrEnum):
...
class MappingProofStatus(StrEnum):
    VERIFIED = "VERIFIED"
    REJECTED = "REJECTED"
    INCOMPLETE = "INCOMPLETE"


class EvidenceMeasurementUnit(StrEnum):
    METRE = "METRE"
...
_MAPPING_ALLOWED_RAW_KINDS: dict[
    MappingProofKind,
    frozenset[RawEvidenceKind],
] = {
    MappingProofKind.ENTITY_IDENTITY: frozenset(
        {RawEvidenceKind.SOURCE_RECORD, RawEvidenceKind.MAPPING_INPUT}
    ),
    MappingProofKind.GEOMETRY: frozenset(
        {RawEvidenceKind.RAW_OBSERVATION, RawEvidenceKind.MAPPING_INPUT}
    ),
    MappingProofKind.SUPPORT: frozenset(
        {RawEvidenceKind.RAW_OBSERVATION, RawEvidenceKind.MAPPING_INPUT}
    ),
    MappingProofKind.CAMERA: frozenset(
        {RawEvidenceKind.RAW_OBSERVATION, RawEvidenceKind.MAPPING_INPUT}
    ),
    MappingProofKind.OBSERVATION_NORMALIZATION: frozenset(
        {RawEvidenceKind.RAW_OBSERVATION, RawEvidenceKind.MAPPING_INPUT}
    ),
    MappingProofKind.RELATION_NORMALIZATION: frozenset(
        {RawEvidenceKind.RAW_OBSERVATION, RawEvidenceKind.MAPPING_INPUT}
    ),
    MappingProofKind.WRITE_BACK: frozenset(
        {RawEvidenceKind.WRITE_BACK, RawEvidenceKind.RAW_OBSERVATION}
    ),
}

_AUDIT_ALLOWED_RAW_KINDS = frozenset(
    {
        RawEvidenceKind.FINAL_AUDIT,
        RawEvidenceKind.RUNTIME_LOG,
        RawEvidenceKind.RAW_OBSERVATION,
        RawEvidenceKind.WRITE_BACK,
    }
)
...
class SourceIdentity(CanonicalModel):
    ...
    native_scene_locator: NativeLocator
...
class EvidenceMeasurement(CanonicalModel):
    ...
    def validate_bounds(self) -> Self:
        ...
        if self.unit is EvidenceMeasurementUnit.FRACTION:
...
class MappingProof(CanonicalModel):
    ...
    def canonicalize_and_validate_proof(self) -> Self:
        ...
        if self.kind is MappingProofKind.ENTITY_IDENTITY and (
        ...
        if self.kind is MappingProofKind.WRITE_BACK:
...
class PreSemanticEvidenceEnvelope(CanonicalModel):
    ...
    raw_evidence_refs: tuple[RawEvidenceRef, ...]
    mapping_proofs: tuple[MappingProof, ...]
    ...
    def _validate_references(self) -> None:
        ...
        if any(
            proof.kind is MappingProofKind.WRITE_BACK for proof in self.mapping_proofs
        ):
            raise ValueError(
        ...
        _require_raw_kinds(
            self.source.raw_evidence_ids,
            raw_by_id,
            allowed=frozenset({RawEvidenceKind.SOURCE_RECORD}),
...
class RuntimeIdentityEvidence(CanonicalModel):
...
class FinalAuditOutcome(CanonicalModel):
    ...
    def canonicalize_and_validate_audit(self) -> Self:
        ...
        elif self.status is AuditStatus.FAIL:
...
class EvidenceEnvelope(CanonicalModel):
    ...
    adapter: AdapterIdentity
    source: SourceIdentity
    native_object_bindings: tuple[NativeObjectBinding, ...]
    raw_evidence_refs: tuple[RawEvidenceRef, ...]
    mapping_proofs: tuple[MappingProof, ...]
    runtime_identities: tuple[RuntimeIdentityEvidence, ...]
    final_audits: tuple[FinalAuditOutcome, ...]
    ...
    def _validate_references(self) -> None:
        ...
        for runtime in self.runtime_identities:
            ...
            _require_raw_kinds(
                runtime.raw_evidence_ids,
                raw_by_id,
                allowed=frozenset({RawEvidenceKind.RUNTIME_LOG}),
        ...
        for audit in self.final_audits:
            ...
            if audit.status is not AuditStatus.NOT_RUN and not any(
                raw_by_id[evidence_id].kind is RawEvidenceKind.FINAL_AUDIT
...
def _require_raw_kinds(
    referenced_ids: tuple[str, ...],
    raw_by_id: dict[str, RawEvidenceRef],
    *,
    allowed: frozenset[RawEvidenceKind],
    label: str,
) -> None:
    incompatible = tuple(
        evidence_id
