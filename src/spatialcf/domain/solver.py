"""Strict domain-only wire contracts for the current continuous-yaw solver.
...
from spatialcf.domain.certificate import OptimalityClaimV2
from spatialcf.domain.edit import CanonicalEdit
from spatialcf.domain.problem import SchemaIdentityV2_3
...
class ContinuousYawResourceUsageV2_8(CanonicalModel):
...
class ContinuousYawSolverConfigV2_8(CanonicalModel):
    ...
    def config_sha256(self) -> Sha256Digest:
        return canonical_sha256(self, domain=_CONFIG_HASH_DOMAIN_V2_8)


class ContinuousYawCandidateRefsV2_8(CanonicalModel):
    ...
    def candidate_refs_sha256(self) -> Sha256Digest:
        return canonical_sha256(self, domain=_CANDIDATE_REFS_HASH_DOMAIN_V2_8)


class ContinuousYawObjectiveCellV2_8(CanonicalModel):
    ...
    def sequence_sha256(
        ...
        return canonical_sha256(ordered, domain=cls._SEQUENCE_HASH_DOMAIN)


class ContinuousYawWitnessEvaluationV2_8(CanonicalModel):
    ...
    def witness_evaluation_sha256(self) -> Sha256Digest:
        return canonical_sha256(self, domain=_WITNESS_HASH_DOMAIN_V2_8)


class ContinuousYawGlobalOptimalityCertificateV2_8(CanonicalModel):
    ...
    def certificate_sha256(self) -> Sha256Digest:
        return canonical_sha256(self, domain=_CERTIFICATE_HASH_DOMAIN_V2_8)
...
class _ContinuousYawSolveResultBaseV2_8(CanonicalModel):
    semantic_problem_sha256: Sha256Digest
    solver_config: ContinuousYawSolverConfigV2_8

    @property
    def solve_result_sha256(self) -> Sha256Digest:
        return canonical_sha256(self, domain=_SOLVE_RESULT_HASH_DOMAIN_V2_8)
...
class ContinuousYawMinimumCostSolveOutcomeV2_8(CanonicalModel):
...
class ContinuousYawSolveVerificationOutcomeV2_8(CanonicalModel):
...
_CONFIG_HASH_DOMAIN_V2_9 = "spatialcf.continuous-yaw-solver-config.v2.9"
_CANDIDATE_REFS_HASH_DOMAIN_V2_9 = "spatialcf.continuous-yaw-candidate-refs.v2.9"
_OBJECTIVE_CELLS_HASH_DOMAIN_V2_9 = "spatialcf.continuous-yaw-objective-cells.v2.9"
_WITNESS_HASH_DOMAIN_V2_9 = "spatialcf.continuous-yaw-witness-evaluation.v2.9"
_CERTIFICATE_HASH_DOMAIN_V2_9 = "spatialcf.continuous-yaw-certificate.v2.9"
_SOLVE_RESULT_HASH_DOMAIN_V2_9 = "spatialcf.continuous-yaw-solve-result.v2.9"
_MAX_DETERMINISTIC_LIMIT_V2_9 = 2**63 - 1

PositiveDeterministicLimitV2_9 = Annotated[
    int,
    Field(strict=True, ge=1, le=_MAX_DETERMINISTIC_LIMIT_V2_9),
]
NonNegativeDeterministicLimitV2_9 = Annotated[
    int,
    Field(strict=True, ge=0, le=_MAX_DETERMINISTIC_LIMIT_V2_9),
]


class ContinuousYawResourceUsageV2_9(CanonicalModel):
    """One cumulative generation ledger for the complete v2.9 solve."""

    domain_operations: NonNegativeDeterministicLimitV2_9
    so2_atomic_steps: NonNegativeDeterministicLimitV2_9
    candidate_cells: NonNegativeDeterministicLimitV2_9
    objective_partition_cells: NonNegativeDeterministicLimitV2_9
    branch_nodes: Literal[0] = 0
    refinement_steps: Literal[0] = 0


class ContinuousYawSolverConfigV2_9(CanonicalModel):
    """Closed public policy for the complete continuous-yaw solve."""

    schema_identity: SchemaIdentityV2_3 = Field(
        default_factory=lambda: SchemaIdentityV2_3(
            schema_name="continuous-yaw-solver-config"
        )
    )
    algorithm_id: Literal["solver:canonical-branch-and-bound-v2"] = (
    ...
    algorithm_version: Literal["algorithm:2.9"] = "algorithm:2.9"
    candidate_config: StrictConvexCandidateCompilerConfigV2_7
    camera_frame_kernel_id: Literal[
    ...
    max_objective_partition_cells: PositiveDeterministicLimitV2_9
    max_branch_nodes: Literal[0] = 0
    max_refinement_steps: Literal[0] = 0
    target_optimality_gap: NonNegativeFiniteFloat
    ...
    @model_validator(mode="after")
    def validate_identity(self) -> Self:
        expected = SchemaIdentityV2_3(schema_name="continuous-yaw-solver-config")
        if self.schema_identity != expected:
            raise ValueError("continuous-yaw v2.9 config identity must be fixed")
        return self

    @property
    def config_sha256(self) -> Sha256Digest:
        return canonical_sha256(self, domain=_CONFIG_HASH_DOMAIN_V2_9)


class ContinuousYawCandidateRefsV2_9(CanonicalModel):
    """Hash chain for original input, projected prefix, camera, and hard stages."""

    semantic_problem_sha256: Sha256Digest
    candidate_problem_sha256: Sha256Digest
    solver_config_sha256: Sha256Digest
    camera_context_sha256: Sha256Digest
    t15_candidate_artifact_sha256: Sha256Digest
    target_candidate_stage_sha256: Sha256Digest | None = None
    visibility_candidate_stage_sha256: Sha256Digest | None = None
...
class ContinuousYawObjectiveCellV2_9(CanonicalModel):
    ...
    _SEQUENCE_HASH_DOMAIN: ClassVar[str] = _OBJECTIVE_CELLS_HASH_DOMAIN_V2_9
...
class ContinuousYawWitnessEvaluationV2_9(CanonicalModel):
    """Fresh point loss and slack enclosure for one concrete edit."""
    ...
    @property
    def witness_evaluation_sha256(self) -> Sha256Digest:
        return canonical_sha256(self, domain=_WITNESS_HASH_DOMAIN_V2_9)


class ContinuousYawGlobalOptimalityCertificateV2_9(CanonicalModel):
    """Directed scalar optimum claim over the complete v2.9 candidate chain."""
    ...
    optimality_claim: OptimalityClaimV2
    epsilon: PositiveFiniteFloat | None = None
    final_resource_usage: ContinuousYawResourceUsageV2_9
...
def _canonicalize_v2_9_objective_cells(
    cells: tuple[ContinuousYawObjectiveCellV2_9, ...],
) -> tuple[ContinuousYawObjectiveCellV2_9, ...]:
    ordered = tuple(sorted(cells, key=lambda cell: cell.cell_id))
    cell_ids = tuple(cell.cell_id for cell in ordered)
    if len(cell_ids) != len(set(cell_ids)):
        raise ValueError("objective cell IDs must be unique")
...
def _validate_v2_9_usage_limits(
    usage: ContinuousYawResourceUsageV2_9,
...
class _ContinuousYawSolveResultBaseV2_9(CanonicalModel):
    semantic_problem_sha256: Sha256Digest
    candidate_problem_sha256: Sha256Digest
...
class ContinuousYawCertifiedSuccessResultV2_9(_ContinuousYawSolveResultBaseV2_9):
    status: Literal[ContinuousYawSolveStatusV2.CERTIFIED_SUCCESS] = (
        ContinuousYawSolveStatusV2.CERTIFIED_SUCCESS
    )
    candidate_refs: ContinuousYawCandidateRefsV2_9
    objective_cells: tuple[ContinuousYawObjectiveCellV2_9, ...] = Field(min_length=1)
    selected_witness: ContinuousYawWitnessEvaluationV2_9
    global_loss_lower_bound: NonNegativeFiniteFloat
    witness_loss_bounds: ObjectiveTermBoundsV2
    final_resource_usage: ContinuousYawResourceUsageV2_9
    ...
    def validate_success(self) -> Self:
        ...
        if refs.semantic_problem_sha256 != self.semantic_problem_sha256:
            raise ValueError("candidate refs problem hash is not closed")
        if refs.candidate_problem_sha256 != self.candidate_problem_sha256:
            raise ValueError("candidate refs projected problem hash is not closed")
        if refs.solver_config_sha256 != config_hash:
            raise ValueError("candidate refs config hash is not closed")
        if (
            refs.target_candidate_stage_sha256 is None
            ...
            raise ValueError("success requires the complete candidate stage chain")
        cells = _canonicalize_v2_9_objective_cells(self.objective_cells)
        object.__setattr__(self, "objective_cells", cells)
        selected_cells = tuple(
            cell
            for cell in cells
        ...
        expected = (
            ...
            config_hash,
            refs.candidate_refs_sha256,
            ContinuousYawObjectiveCellV2_9.sequence_sha256(cells),
            self.selected_witness.witness_evaluation_sha256,
            self.selected_witness.edit.edit_sha256,
        )
        actual = (
            certificate.semantic_problem_sha256,
            certificate.candidate_problem_sha256,
            certificate.solver_config_sha256,
            certificate.candidate_refs_sha256,
            certificate.objective_cells_sha256,
            certificate.witness_evaluation_sha256,
...
class ContinuousYawProvenUnsatResultV2_9(_ContinuousYawSolveResultBaseV2_9):
    status: Literal[ContinuousYawSolveStatusV2.PROVEN_UNSAT] = (
        ContinuousYawSolveStatusV2.PROVEN_UNSAT
    )
    candidate_refs: ContinuousYawCandidateRefsV2_9
...
class ContinuousYawUncertifiedResultV2_9(_ContinuousYawSolveResultBaseV2_9):
    status: Literal[ContinuousYawSolveStatusV2.UNCERTIFIED] = (
        ContinuousYawSolveStatusV2.UNCERTIFIED
    )
    uncertified_reason: UncertifiedReasonV2
    candidate_refs: ContinuousYawCandidateRefsV2_9 | None = None
    objective_cells: tuple[ContinuousYawObjectiveCellV2_9, ...] = ()
    final_resource_usage: ContinuousYawResourceUsageV2_9 | None = None
    finding_codes: tuple[str, ...] = Field(min_length=1)
    ...
    def validate_uncertified(self) -> Self:
        ...
        if refs is not None and (
            refs.semantic_problem_sha256 != self.semantic_problem_sha256
            or refs.candidate_problem_sha256 != self.candidate_problem_sha256
            or refs.solver_config_sha256 != self.solver_config.config_sha256
        ):
            raise ValueError("uncertified candidate refs are not closed")
        cells = _canonicalize_v2_9_objective_cells(self.objective_cells)
...
ContinuousYawSolveResultV2_9: TypeAlias = Annotated[
...
class ContinuousYawMinimumCostSolveOutcomeV2_9(CanonicalModel):
    """Fresh-solve result plus exact non-capability generation telemetry."""

    result: ContinuousYawSolveResultV2_9 | None
    finding_codes: tuple[str, ...] = ()
    cumulative_generation_usage: ContinuousYawResourceUsageV2_9 | None = None
    proposal_count: NonNegativeDeterministicLimitV2_9 = 0
    evaluated_proposal_count: NonNegativeDeterministicLimitV2_9 = 0
    ...
    def validate_outcome(self) -> Self:
        ...
        if type(result) is ContinuousYawCertifiedSuccessResultV2_9:
            if findings:
                raise ValueError("certified success cannot carry findings")
            if not 1 <= self.proposal_count <= len(result.objective_cells):
                raise ValueError(
                    "success requires at least one and at most one proposal per "
                    "objective cell"
                )
            if self.evaluated_proposal_count != self.proposal_count:
                raise ValueError("success must evaluate every proposal")
...
class ContinuousYawSolveVerificationOutcomeV2_9(CanonicalModel):
    """Fresh replay comparison outcome; verified refs exist only on a match."""
    ...
    submitted_solve_result_sha256: Sha256Digest | None = None
    verified_status: ContinuousYawSolveStatusV2 | None = None
    replay_generation_usage: ContinuousYawResourceUsageV2_9 | None = None
    proposal_count: NonNegativeDeterministicLimitV2_9 = 0
    evaluated_proposal_count: NonNegativeDeterministicLimitV2_9 = 0
    uncertified_reason: UncertifiedReasonV2 | None = None
    finding_codes: tuple[str, ...] = ()
