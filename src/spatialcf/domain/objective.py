"""Platform-neutral current minimum-total-cost objective contracts."""
...
from spatialcf.domain.constraints import RelationAxis
...
class PairAxisKeyV2(CanonicalModel):
...
class ObjectCameraKeyV2(CanonicalModel):
...
class PairAxisWeightV2(CanonicalModel):
    key: PairAxisKeyV2
    damage_weight: PositiveFiniteFloat


class ObjectCameraWeightV2(CanonicalModel):
    key: ObjectCameraKeyV2
    change_weight: PositiveFiniteFloat


class SafetySlackComponentV2(CanonicalModel):
...
class ConstraintSafetyTargetV2(CanonicalModel):
...
class TranslationTermV2(CanonicalModel):
...
class RelationDamageTermV2(CanonicalModel):
...
class VisibilityChangeTermV2(CanonicalModel):
...
class SafetyAggregationV2(CanonicalModel):
...
class SafetyMarginTermV2(CanonicalModel):
    weight: NonNegativeFiniteFloat
    normalizer: PositiveFiniteFloat
...
class ObjectiveSpecV2(CanonicalModel):
    ...
    def objective_spec_sha256(self) -> str:
        ...
        return canonical_sha256(self, domain=_OBJECTIVE_HASH_DOMAIN)
