"""Auditable current domain and objective partition artifacts.
...
from spatialcf.domain.objective import PairAxisKeyV2, SafetySlackUnitV2
...
class PlanarRegionBoundV2(CanonicalModel):
...
class PlanarDomainBoundsV2(CanonicalModel):
...
class CandidateDomainVariableV2(CanonicalModel):
    ...
    baseline_anchor_world_xy_m: Vec2


class CompilationResourceUsageV2(CanonicalModel):
...
class ConstraintDomainShrinkStepV2(CanonicalModel):
...
class CandidateDomainArtifactV2(CanonicalModel):
    ...
    def candidate_domain_artifact_sha256(self) -> Sha256Digest:
        return canonical_sha256(self, domain=_CANDIDATE_DOMAIN_HASH_DOMAIN)


class ConstraintSlackV2(CanonicalModel):
...
class RelationDamageBoundV2(CanonicalModel):
...
class RelationCostCellV2(CanonicalModel):
    cell_id: CanonicalId
...
class RelationCostPartitionV2(CanonicalModel):
    schema_identity: SchemaIdentityV2 = Field(
...
class NonNegativeIntervalV2(CanonicalModel):
...
class ObjectiveTermBoundsV2(CanonicalModel):
...
class ObjectivePartitionCellV2(CanonicalModel):
    cell_id: CanonicalId
...
class ObjectivePartitionArtifactV2(CanonicalModel):
    schema_identity: SchemaIdentityV2 = Field(
        default_factory=lambda: SchemaIdentityV2(
...
class SchemaIdentityV2_2(CanonicalModel):
...
class ObjectPoseV2_2(ObjectPose):
    world_from_object: DirectedYawIntervalTransformV2_2


class CanonicalObjectV2_2(CanonicalObject):
    pose: ObjectPoseV2_2


class GeometryInstanceV2_2(CanonicalModel):
...
class SupportSurfaceFactV2_2(SupportSurfaceFact):
    anchor_from_surface: DirectedYawIntervalTransformV2_2


class PinholeCameraV2_2(PinholeCamera):
    world_to_camera: DirectedYawIntervalTransformV2_2


class CanonicalSceneV2_2(CanonicalScene):
...
class SemanticProblemV2_2(SemanticProblemV2):
    ...
    def semantic_problem_sha256(self) -> str:
        return canonical_sha256(self, domain=_PROBLEM_HASH_DOMAIN_V2_2)


class StrictConvexCandidateCompilerConfigV2_5(CanonicalModel):
    ...
    def config_sha256(self) -> str:
        return canonical_sha256(self, domain=_CONFIG_HASH_DOMAIN_V2_5)


class StrictConvexCandidateCompilerConfigV2_6(CanonicalModel):
    ...
    @property
    def config_sha256(self) -> str:
        return canonical_sha256(self, domain=_CONFIG_HASH_DOMAIN_V2_6)


class StrictConvexCandidateCompilerConfigV2_7(CanonicalModel):
    ...
    @property
    def config_sha256(self) -> str:
        return canonical_sha256(self, domain=_CONFIG_HASH_DOMAIN_V2_7)
