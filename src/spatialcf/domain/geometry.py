"""Platform-neutral current solver geometry facts.
...
class PlanarRingV2(CanonicalModel):
    """A simple canonical ring without a repeated closing vertex."""

    winding: RingWindingV2
    vertices: tuple[Vec2, ...] = Field(min_length=3)
    ...
    def validate_and_canonicalize_ring(self) -> Self:
        ...
        object.__setattr__(
            self,
            "vertices",
            tuple(Vec2(x=x, y=y) for x, y in canonical),
        )
        return self


class PlanarPolygonComponentV2(CanonicalModel):
    ...
    def validate_and_canonicalize_component(self) -> Self:
        ...
        ordered_holes = tuple(sorted(self.holes, key=canonical_json_bytes))
...
class PlanarRegionV2(CanonicalModel):
    ...
    def validate_and_canonicalize_components(self) -> Self:
        ordered = tuple(sorted(self.components, key=canonical_json_bytes))
...
class UprightBox3DV2(CanonicalModel):
...
class ExtrudedPlanarPolygonV2(CanonicalModel):
...
class GeometryInstanceV2(CanonicalModel):
...
class CollisionBodyFactV2(CanonicalModel):
...
class DirectedYawIntervalTransformV2_2(CanonicalModel):
