"""Closed, platform-neutral current semantic problem contract."""
...
from spatialcf.domain.geometry import GeometryApproximationV2, GeometryRoleV2
...
from spatialcf.domain.scene import BaselineObservation, CanonicalScene, PinholeCamera
...
class SemanticProblemV2(CanonicalModel):
    ...
    visibility_semantics: VisibilitySemantics
    ...
    def semantic_problem_sha256(self) -> str:
        ...
        return canonical_sha256(self, domain=_PROBLEM_HASH_DOMAIN)
    ...
    def _validate_position_references(self) -> None:
        ...
        if (
            ...
            is not PositionRegionInterpretation.SUBJECT_OCCUPANCY_CONTAINED
        ):
            return
        ...
        expected_components[target_relation.constraint_id] = {
            (
                ...
                (
                    ...
                    if definition.unit is MeasurementUnit.PIXEL
...
def _possible_values(facts: FactSetV2) -> tuple[CanonicalModel, ...]:
    if facts.availability is not FactAvailabilityV2.KNOWN:
        return ()
...
def _guaranteed_values(facts: FactSetV2) -> tuple[CanonicalModel, ...]:
    if facts.availability is not FactAvailabilityV2.KNOWN:
        return ()
    if facts.completeness is FactCompletenessV2.OUTER_BOUND:
        return ()
...
def _ids(values: Iterable[CanonicalModel], id_field: str) -> set[str]:
...
def _observation_key(observation: BaselineObservation) -> tuple[str, str, str, str]:
    return (
...
def _validate_required_observations(
    facts: FactSetV2[BaselineObservation],
...
_CURRENT_CAMERA_MODEL_NAMES = frozenset(
    {
        "SchemaIdentityV2_3",
...
_CURRENT_CAMERA_MODEL_INSTALL_LOCK = RLock()


def _install_current_camera_problem_models() -> None:
    """Resolve the one artifacts/problem inheritance cycle on first use."""

    if "SemanticProblemV2_3" in globals():
        return
    with _CURRENT_CAMERA_MODEL_INSTALL_LOCK:
        if "SemanticProblemV2_3" in globals():
            return
        _build_current_camera_problem_models_locked()


def _build_current_camera_problem_models_locked() -> None:
    """Build the complete V2.3 family while its installation lock is held."""

    from spatialcf.domain.artifacts import CanonicalSceneV2_2, SemanticProblemV2_2

    class SchemaIdentityV2_3(CanonicalModel):
        """Identity carried only by Canonical 2.3 root contracts."""

        schema_name: CanonicalId
        schema_version: Literal["2.3"] = "2.3"

    SchemaIdentityV2_3.__qualname__ = SchemaIdentityV2_3.__name__
    globals()[SchemaIdentityV2_3.__name__] = SchemaIdentityV2_3

    class UprightWorldToCameraTransformV2_3(CanonicalModel):
    ...
    UprightWorldToCameraTransformV2_3.__qualname__ = (
        UprightWorldToCameraTransformV2_3.__name__
    )
    globals()[UprightWorldToCameraTransformV2_3.__name__] = (
        UprightWorldToCameraTransformV2_3
    )

    class PinholeCameraV2_3(PinholeCamera):
        """Pinhole camera whose frame uses the Canonical 2.3 upright basis."""

        world_to_camera: UprightWorldToCameraTransformV2_3

    PinholeCameraV2_3.__qualname__ = PinholeCameraV2_3.__name__
    globals()[PinholeCameraV2_3.__name__] = PinholeCameraV2_3
    ...
    CanonicalSceneV2_3.__qualname__ = CanonicalSceneV2_3.__name__
    globals()[CanonicalSceneV2_3.__name__] = CanonicalSceneV2_3
    ...
    class SemanticProblemV2_3(SemanticProblemV2_2):
        ...
        @property
        def semantic_problem_sha256(self) -> str:
            return canonical_sha256(self, domain=_PROBLEM_HASH_DOMAIN_V2_3)

    SemanticProblemV2_3.__qualname__ = SemanticProblemV2_3.__name__
    globals()[SemanticProblemV2_3.__name__] = SemanticProblemV2_3


def __getattr__(name: str) -> object:
    if name not in _CURRENT_CAMERA_MODEL_NAMES:
        raise AttributeError(f"module {__name__!r} has no attribute {name!r}")
    _install_current_camera_problem_models()
    return globals()[name]
