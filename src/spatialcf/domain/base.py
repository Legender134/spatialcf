...
class SchemaIdentityV2(CanonicalModel):
...
class Quaternion(CanonicalModel):
...
class RigidTransformV2(CanonicalModel):
...
class NumericPolicyV2(CanonicalModel):
...
class UncertaintyCalculusV2(CanonicalModel):
...
class UncertaintyBudgetV2(CanonicalModel):
...
FactT = TypeVar("FactT", bound=CanonicalModel)


class FactSetV2(CanonicalModel, Generic[FactT]):
    ...
    def _canonicalize_fact_values(
        ...
        if any(not isinstance(value, CanonicalModel) for value in values):
