...
from spatialcf.domain.constraints import (
    ...
    MeasurementOperandOrder,
    Relation,
    RelationAxis,
    RelationDefinition,
    RelationMeasurement,
    RelationRepresentativePoint,
...
from spatialcf.domain.objective import PairAxisKeyV2
...
def _exact_damage_indicator(
    ...
    try:
        if key.axis is RelationAxis.HORIZONTAL:
        ...
        elif key.axis is RelationAxis.DEPTH:
            baseline = (
...
def _axis_definitions(
    ...
    if len(definitions) != 2 or any(
        ...
        or item.operand_order is not MeasurementOperandOrder.FIRST_MINUS_SECOND
        or item.boundary_policy is not BoundaryPolicy.CLOSED
    ...
    if axis is not RelationAxis.DISTANCE and any(
        ...
        is not RelationRepresentativePoint.RELATION_GEOMETRY_VOLUME_CENTROID
        for item in definitions
    ):
        return None
...
def _horizontal_intervals(
    context: _ExactContextV2,
    key: PairAxisKeyV2,
    first: CanonicalObject,
    first_geometry: GeometryInstanceV2,
    second: CanonicalObject,
...
def _linear_label_set(
    ...
    lower: Fraction,
    upper: Fraction,
) -> frozenset[Relation] | None:
    labels: set[Relation] = set()
    for definition in definitions:
        threshold = Fraction.from_float(definition.threshold)
        _require_fraction_size(threshold)
        if definition.comparator is MeasurementComparator.LESS_THAN:
...
def _camera_is_exact_identity(camera: PinholeCamera) -> bool:
    intrinsics = camera.intrinsics_row_major
    return (
        _is_identity_transform(camera.world_to_camera)
        and camera.distortion_model is CameraDistortionModel.NONE
        ...
        and camera.matrix_layout is CameraMatrixLayout.ROW_MAJOR
        and camera.camera_axes is CameraAxes.X_RIGHT_Y_DOWN_Z_FORWARD
        and camera.pixel_convention is CameraPixelConvention.CENTER_AT_HALF
        and camera.depth_convention is CameraDepthConvention.POSITIVE_Z_FORWARD
    )
