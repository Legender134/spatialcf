...
class DirectionalTargetCandidateStageV2_9:
    ...
    def __post_init__(self) -> None:
        ...
        if type(self.target_relation) is not Relation:
            raise TypeError("target_relation must be an exact Relation")
    ...
    *,
    ...
    try:
        ...
        with warnings.catch_warnings():
            ...
            if target.relation_after in (Relation.NEAR, Relation.FAR):
                ...
                inner = _clip_bracket_complex(
                    ...
                    Fraction.from_float(definition.threshold)
                    if definition.measurement is RelationMeasurement.CAMERA_DEPTH_DELTA
                    else Fraction(),
                    inner=True,
...
def _strict_inputs(
    ...
    with warnings.catch_warnings():
        ...
        replayed = prepare_camera_independent_candidate_problem(original)
...
def _extract_target(
    ...
    expected_measurement = {
        Relation.LEFT: RelationMeasurement.PROJECTED_CENTER_DELTA_X,
        Relation.RIGHT: RelationMeasurement.PROJECTED_CENTER_DELTA_X,
        Relation.FRONT: RelationMeasurement.CAMERA_DEPTH_DELTA,
        Relation.BEHIND: RelationMeasurement.CAMERA_DEPTH_DELTA,
        Relation.NEAR: RelationMeasurement.SHAPE_GAP_XY,
        Relation.FAR: RelationMeasurement.SHAPE_GAP_XY,
    }[target.relation_after]
    expected_comparator = (
        MeasurementComparator.LESS_THAN
        if target.relation_after in (Relation.LEFT, Relation.FRONT, Relation.NEAR)
...
def _directional_affine(
    ...
    if measurement is RelationMeasurement.CAMERA_DEPTH_DELTA:
    ...
    subject_bounds = bound_world_point_in_upright_camera(
        camera,
        world_xyz=subject,
    ...
    reference_bounds = bound_world_point_in_upright_camera(
        camera,
        world_xyz=reference,
    ...
    if measurement is not RelationMeasurement.PROJECTED_CENTER_DELTA_X:
    ...
    if relation not in (Relation.LEFT, Relation.RIGHT):
    ...
    *,
    ...
    for upstream_cell in upstream.cells:
        for x_nonnegative, y_nonnegative in sign_cells:
            ...
            if comparator is MeasurementComparator.LESS_THAN:
                coefficients = (
            ...
            elif comparator is MeasurementComparator.GREATER_THAN:
                coefficients = (
