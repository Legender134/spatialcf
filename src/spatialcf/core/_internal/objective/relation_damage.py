...
from spatialcf.core._internal.objective.relation_partition import (
    _axis_definitions,
    _distance_label_set,
    _linear_label_set,
)
...
from spatialcf.domain.constraints import RelationAxis, RelationDefinition
    ...
    def stage_sha256(self) -> str:
        ...
        for cell in self.cells:
            ...
            for bound in cell.vector:
                digest.update(canonical_json_bytes(bound))
...
def _linear_measurement(
    ...
    for object_id in (key.first_object_id, key.second_object_id):
        ...
        point = bound_world_point_in_upright_camera(
            context.camera,
            world_xyz=centroid,
        ...
        if key.axis is RelationAxis.HORIZONTAL and not point.positive_depth:
            raise _UnsupportedRelationDamageV2_9(
