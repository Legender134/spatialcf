...
def _position_components(
    ...
    if len(constraint.known_free_space_fact_ids) > 1 or (
        ...
        is not RegionAggregation.INTERSECTION
    ):
    ...
    if constraint.known_free_space_fact_ids:
        ...
        if (
            ...
            or free.boundary_policy is not RegionBoundaryPolicy.CLOSED
...
def _objects(problem: SemanticProblemV2) -> dict[str, CanonicalObject]:
...
def _identity_rotation(rotation: Quaternion) -> bool:
