...
from spatialcf.domain.scene import BaselineObservation
...
def _extract_visibility_subset(
    ...
    image_area_formula = next(
        ...
        if definition.kind is VisibilityMetricKind.IMAGE_AREA_FRACTION
    )
...
def _classify_cell(
    ...
    camera: UprightCameraContextV2_9,
    constraint: VisibilityConstraint,
    image_area_formula: VisibilityMetricFormula,
    budget: SO2AtomicBudgetV2,
...
def _image_area_fraction_lower_bound_v2_9(
    ...
    *,
    ...
    if (
        formula
        is VisibilityMetricFormula.VISIBLE_CLIPPED_PROJECTED_AREA_OVER_IMAGE_AREA
    ):
        return min(
            Fraction(1),
            focal_x_px
