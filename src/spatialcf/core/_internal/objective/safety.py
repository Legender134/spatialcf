...
from spatialcf.core._internal.kernels.rect import ExactAxisAlignedRectV2
...
from spatialcf.core._internal.objective.numeric import (
...
from spatialcf.core._internal.objective.safety_bounds import ConstraintSlackBoundsV2
...
from spatialcf.domain.base import UncertaintyBudgetV2, Vec3
    ...
    *,
    ...
    try:
        ...
        with warnings.catch_warnings():
            ...
            camera = compile_upright_camera_context(
                problem,
...
def _position_components(problem, target, dx, dy, *, cell):
    ...
    if (
        constraint.boundary_policy is not BoundaryPolicy.CLOSED
        or constraint.region_interpretation
        is not PositionRegionInterpretation.SUBJECT_ANCHOR_LOCUS
        or constraint.workspace_aggregation is not RegionAggregation.INTERSECTION
    ...
    if (
        ...
        or workspace.boundary_policy is not RegionBoundaryPolicy.CLOSED
...
def _collision_operand(problem, body):
    ...
    if (
        ...
        or geometry.uncertainty != UncertaintyBudgetV2()
        or type(geometry.shape) is not UprightBox3DV2
        or geometry.anchor_from_geometry.translation != Vec3(x=0.0, y=0.0, z=0.0)
...
def _target_components(
    ...
    if key.axis in (RelationAxis.HORIZONTAL, RelationAxis.DEPTH):
    ...
    elif definition.comparator is MeasurementComparator.GREATER_THAN:
