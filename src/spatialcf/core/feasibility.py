...
from spatialcf.domain.scene import Camera, Scene, SceneObject
...
def _intersect_half_plane(
    ...
    if keep_greater:
        ...
        constant -= tolerance
    return _polygonal(
        region.intersection(_half_plane_mask(region, nx, ny, constant, keep_greater))
    )


def _camera_row(
    camera: Camera, row: int, subject_z: float
    ...
    return (
        matrix[start],
        matrix[start + 1],
        matrix[start + 2] * subject_z + matrix[start + 3],
    )


def _require_query_views(
    subject: SceneObject, reference: SceneObject, camera_id: str
) -> None:
    if subject.views.get(camera_id) is None or reference.views.get(camera_id) is None:
        raise ValueError(
            "camera-relative feasible regions require subject and reference query views"
        )
        ...
        if spec.relation_after in {
            ...
            if spec.relation_after in {Relation.LEFT, Relation.RIGHT}:
                region = self._left_right_region(
                    region, subject, reference, camera, spec
                )
            else:
                region = self._front_behind_region(
                    region, subject, reference, camera, spec
                )
        elif spec.relation_after is Relation.NEAR:
            configuration = _configuration_obstacle(
                obb_footprint(reference.obb), relative_vertices
            )
            near_locus = _convex_minkowski_sum(
                configuration,
                list(_regular_disk_polygon(0.50, circumscribed=True).exterior.coords)[
                    :-1
                ],
            )
            ...
            far_exclusion = _convex_minkowski_sum(
                configuration,
                list(_regular_disk_polygon(1.50, circumscribed=False).exterior.coords)[
                    :-1
                ],
            )
