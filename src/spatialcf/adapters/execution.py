"""Adapter-owned execution values for grounded counterfactual observation.

The pure core never imports these records. Simulator actions, physics engines,
renderer events, and platform-specific tolerances belong in implementations of
...
class CandidateExecution(Generic[EvidenceT_co]):
    ...
    def __post_init__(self) -> None:
        ...
        if (
            len(set(self.errors)) != len(self.errors)
            or tuple(sorted(self.errors)) != self.errors
        ):
        ...
        if self.status is CandidateExecutionStatus.OBSERVED:
            ...
            if (
                self.observed_before_scene is not None
                or self.observed_scene is not None
            ):
