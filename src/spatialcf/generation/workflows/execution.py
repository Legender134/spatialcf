"""Final reusable partial-batch execution and immutable publication."""
...
from spatialcf.adapters.base import EnvironmentAdapter
...
from spatialcf.generation.execution.audit import AuditExecution, EndpointAuditRejected
...
class BatchAttempt(CanonicalModel):
...
def execute_batch(
    ...
    *,
    ...
    runner: Callable[..., AuditExecution],
) -> BatchSummary:
...
def _populate_batch_transaction(
    ...
    *,
    ...
    for index, request in enumerate(manifest.requests):
        ...
        require_verified = getattr(runner, "require_verified", None)
        if require_verified is not None:
            require_verified(request.request_id)
    ...
    with bound_child_directory(
        ...
        if canonical_json_bytes(final_summary) != canonical_json_bytes(summary):
