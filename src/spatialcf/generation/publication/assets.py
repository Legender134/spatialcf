...
from spatialcf.generation.execution.correspondence import legacy_sha256
from spatialcf.verification.artifacts import (
...
class ReturnedAssetRef(CanonicalModel):
    phase: AssetPhase
    kind: AssetKind
...
class InstancePixelCount(CanonicalModel):
    object_id: CanonicalId
    pixel_count: int = Field(strict=True, ge=0)


class AssetBundle(CanonicalModel):
    ...
    def competition_native_asset_bundle_sha256(self) -> Sha256Digest:
        return canonical_sha256(self, domain=_BUNDLE_HASH_DOMAIN)
...
def publish_asset_bundle(
    ...
    *,
    ...
    with warnings.catch_warnings():
        ...
        _require_execution_observation_closure(checked)
            ...
            with parent.create_staging() as transaction:
                try:
                    ...
                    with bound_child_directory(
                        ...
                        if canonical_json_bytes(verified) != canonical_json_bytes(
                            bundle
                        ):
                            raise RuntimeError(
...
def load_asset_bundle(root: Path) -> AssetBundle:
    ...
    with warnings.catch_warnings():
        ...
        with bound_absolute_directory(root) as descriptor:
            ...
            revalidate_retained_tree(descriptor, entries)
            return bundle
...
def _verify_bundle_tree_fd_with_digests(
    ...
    ) != canonical_json_bytes(expected_run.model_dump(mode="json", warnings="error")):
...
def _read_bundle_metadata_fd(
    ...
    entries = snapshot_exact_artifact_tree(descriptor, regular_names=expected_names)
    bundle_payload = read_retained_artifact(
        descriptor,
        "bundle.json",
        _MAX_METADATA_BYTES,
...
def _verify_bundle_payloads_fd(
    ...
    *,
    ...
    for key, filename in _ASSET_FILENAMES.items():
        ...
        payload = read_retained_artifact(
            descriptor,
            filename,
    ...
    checksum_payload = read_retained_artifact(
        descriptor,
        "checksums.sha256",
        _MAX_METADATA_BYTES,
        expected_stat=entries["checksums.sha256"],
    )
    payload_digests = _bundle_payload_digests(bundle, bundle_payload)
    require_exact_checksum_ledger(checksum_payload, payload_digests)
    digests = retained_sha256_digests(
        {
            "bundle.json": bundle_payload,
            "checksums.sha256": checksum_payload,
            **{_ASSET_FILENAMES[key]: payload for key, payload in payloads.items()},
        }
    )
...
def _observation_payloads(
    before: AdapterObservation, after: AdapterObservation
...
def _require_execution_observation_closure(execution: AuditExecution) -> None:
    before = execution.before_observation
    fresh_before = execution.fresh_before_observation
    after = execution.after_observation
    _require_observation(before, "before")
    _require_observation(fresh_before, "fresh before")
    _require_observation(after, "after")
    if before.scene != execution.run.source_scene:
        raise ValueError("before observation scene does not close its audit run")
    if fresh_before.scene != execution.run.fresh_source_scene:
        raise ValueError("fresh before observation scene does not close its audit run")
    if (
        before.rgb_png,
        before.depth_npy,
        before.instance_png,
        before.pointcloud_ply,
    ) != (
        fresh_before.rgb_png,
        fresh_before.depth_npy,
        fresh_before.instance_png,
        fresh_before.pointcloud_ply,
    ):
        raise ValueError("before observation assets do not close its audit run")
    if observation_sha256(before) != execution.run.before_observation_sha256:
        raise ValueError("before observation digest does not close its audit run")
    if (
        observation_sha256(fresh_before)
        != execution.run.fresh_before_observation_sha256
    ):
        raise ValueError("fresh before observation digest does not close its audit run")
    if legacy_sha256(after.scene) != execution.run.native_audit.observed_scene_sha256:
        raise ValueError("after observation scene does not close its audit run")
    if observation_sha256(after) != execution.run.native_audit.after_observation_sha256:
        raise ValueError("after observation digest does not close its audit run")


def _pixel_counts(observation: AdapterObservation) -> tuple[InstancePixelCount, ...]:
    return tuple(
        InstancePixelCount(object_id=object_id, pixel_count=count)
        for object_id, count in observation.instance_pixel_counts
    )
...
def _require_observation(observation: AdapterObservation, label: str) -> None:
...
def _adapter_observation_errors(
    ...
    assets = (
        ...
        ("instance", observation.instance_png, observation.instance_png_sha256),
        (
            "pointcloud",
    ...
    counts = dict(observation.instance_pixel_counts)
...
def _checksum_payload(bundle: AssetBundle, bundle_payload: bytes) -> bytes:
    return canonical_checksum_ledger(_bundle_payload_digests(bundle, bundle_payload))


def _bundle_payload_digests(
    bundle: AssetBundle,
    bundle_payload: bytes,
) -> dict[str, str]:
