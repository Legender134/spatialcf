"""Final source-campaign execution over immutable partial batches."""
...
import spatialcf.generation.planning.campaign as planning_campaign
...
from spatialcf.generation.planning.models import EndpointPlan
...
class SourceExecutionSummary(CanonicalModel):
...
def _checked_source_plan(plan: object) -> SourcePlan:
    if type(plan) is RetainedSourcePlan:
        ...
        payload_sha256 = hashlib.sha256(
            canonical_json_bytes(checked.model_dump(mode="json", warnings="error"))
            + b"\n"
        ).hexdigest()
