...
from spatialcf.core._internal.kernels.convex_translation import RationalPoint2V2
from spatialcf.core.feasibility import (
...
from spatialcf.domain.scene import Scene, SceneObject, SubjectPositionRegion
...
def _solver_success_mismatch(
    ...
    try:
        ContinuousYawCertifiedSuccessResultV2_9.model_validate(result, strict=True)
    except (TypeError, ValueError):
        return "endpoint_plan:solver_certificate_mismatch"
    return None
...
class _PatchLocus:
    ...
    position_geometry: BaseGeometry
    ...
    def __post_init__(self) -> None:
        ...
        if not isinstance(self.position_geometry, BaseGeometry):
            raise TypeError("patch locus position geometry must be exact")
...
def _single_patch_loci_from_prepared(
    ...
    subject = scene.object_by_id(subject_placement.object_id)
    relative_vertices = _relative_vertices(subject)
    return tuple(
        _PatchLocus(
            patch=patch,
            position_geometry=_center_locus(
                box(
                    patch.x_min,
                    patch.z_min,
                    patch.x_max,
        ...
        for patch in checked_evidence.patches
    )
...
def endpoint_workspace_within_position_region(
    ...
    return _endpoint_workspace_within_geometry(
        subject,
        subject_position_region_geometry(region),
        workspace,
    )


def _endpoint_workspace_within_geometry(
    subject: SceneObject,
    pivot_locus: BaseGeometry,
    workspace: EndpointWorkspace,
) -> bool:
    """Prove one OBB-center workspace lies in a persisted anchor locus."""

    if type(subject) is not SceneObject:
        raise TypeError("workspace subject must be exact")
    if not isinstance(pivot_locus, BaseGeometry):
        raise TypeError("workspace position geometry must be exact")
    if type(workspace) is not EndpointWorkspace:
        raise TypeError("workspace must be exact")
...
def _assign_patch_owned_candidates(
    *,
    ...
    geometries = tuple((item, item.position_geometry) for item in canonical_loci)
...
def plan_endpoint(
    ...
    *,
    ...
    subject_placement = placement.subject_placement
    if subject_placement.position_region is None:
        raise ValueError("patch-bound evidence has no persisted placement region")
    patch_locus_union: BaseGeometry = GeometryCollection()
    for patch_locus in patch_loci:
        patch_locus_union = patch_locus_union.union(patch_locus.position_geometry)
    if not patch_locus_union.equals(
        subject_position_region_geometry(subject_placement.position_region)
    ):
        raise ValueError("patch loci do not bind persisted placement region")
    subject = scene.object_by_id(intervention.subject_id)
    ...
    for candidate_index, candidate in enumerate(candidates):
        ...
        for candidate_radius in radii:
            ...
            if endpoint_workspace is None or not (
                _endpoint_workspace_within_geometry(
                    subject,
                    locus.position_geometry,
                    endpoint_workspace,
                )
            ):
