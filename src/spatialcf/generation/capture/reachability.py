...
from spatialcf.domain.base import CanonicalModel, Sha256Digest
from spatialcf.domain.constraints import (
    MeasurementComparator,
    RelationMeasurement,
)
from spatialcf.domain.constraints import (
    Relation as ConstraintRelation,
)
...
def build_competition_native_candidate_target_reachability_v2_9_4(
    *,
    ...
    payload = {
        ...
        "reachable_native_positions_sha256": canonical_sha256(
            coordinate_payload,
            domain=_REACHABLE_POSITIONS_HASH_DOMAIN_V2_9_4,
        ),
        "status": (
    ...
    return CompetitionNativeCandidateTargetReachabilityV2_9_4(
        **payload,
        target_reachability_sha256=canonical_sha256(
            payload,
            domain=_TARGET_REACHABILITY_HASH_DOMAIN_V2_9_4,
        ),
    )
...
def competition_native_target_only_relation_after_native_coordinates_v2_9_4(
    ...
    try:
        if target.relation_after in (
            ConstraintRelation.LEFT,
            ConstraintRelation.RIGHT,
            ConstraintRelation.FRONT,
            ConstraintRelation.BEHIND,
        ):
            return _target_only_directional_native_coordinates_v2_9_4(
                problem, native_points
            )
        if target.relation_after in (ConstraintRelation.NEAR, ConstraintRelation.FAR):
...
def _target_only_directional_native_coordinate_margins_v2_9_4(
    ...
    projected = prepare_camera_independent_candidate_problem(problem)
    ...
    for point in native_points:
        ...
        if definition.measurement is RelationMeasurement.PROJECTED_CENTER_DELTA_X:
        ...
        elif definition.measurement is RelationMeasurement.CAMERA_DEPTH_DELTA:
...
def _target_only_shape_gap_native_coordinates_v2_9_4(
    ...
    if overlap.kind is ConvexTranslationDomainKindV2.IDENTITY:
        ...
        return frozenset()
    if relation is ConstraintRelation.FAR:
