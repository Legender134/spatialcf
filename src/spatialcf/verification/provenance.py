...
from spatialcf.domain.scene import Vec3
...
class _ObjectiveBreakdownLike(Protocol):
...
class _SolveResultLike(Protocol):
    ...
    score: _ObjectiveBreakdownLike | None
    quality: QualityTier
...
class GenerationProvenance(_FrozenEvidence):
    ...
    def exact_generator_config(
        ...
        if generator == "spatialcf":
            ...
            if type(value["refine_steps"]) is not list or any(
                type(item) is not float for item in value["refine_steps"]
            ):
...
class ObjectiveEvidence(_FrozenEvidence):
    ...
    @classmethod
    def from_score(cls, score: _ObjectiveBreakdownLike) -> ObjectiveEvidence:
        return cls(
...
class GeneratorResultEvidence(_FrozenEvidence):
    ...
    @classmethod
    def from_result(cls, result: _SolveResultLike) -> GeneratorResultEvidence:
        return cls(
